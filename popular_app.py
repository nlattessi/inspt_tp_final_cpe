...
def populate():
    ...
    for i in range(0, 5):
        add_vendedor(legajo=faker.unix_time(),
                     nombre=faker.first_name(), apellido=faker.last_name())
...
def crear_superuser():
    User.objects.create_superuser('admin', 'admin@admin.com', 'pass')
...
def add_estado(nombre):
    ...
    for estado in estados:
        e = Estado.objects.get_or_create(nombre=estado)[0]
    return e
...
def add_handheld(numero_de_serie, modelo, marca):
    h = Handheld.objects.get_or_create(numero_de_serie=numero_de_serie,
                                       ...
                                       centro_distribucion=CentroDistribucion.objects.order_by(
                                           '?')[0],
                                       )
    return h
...
def add_incidente(descripcion, solucion):
    i = Incidente.objects.get_or_create(descripcion=descripcion,
                                        solucion=solucion,
                                        handheld=Handheld.objects.order_by(
                                            '?')[0],
                                        vendedor=Vendedor.objects.order_by(
                                            '?')[0],
